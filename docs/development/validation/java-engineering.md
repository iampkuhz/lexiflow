...
```bash
python3 -m scripts.environment.java_exec backend/gradlew -p backend :apps:api:checkstyleMain --rerun-tasks --no-build-cache
python3 -m scripts.environment.java_exec backend/gradlew -p backend :apps:api:test --rerun-tasks --no-build-cache
python3 -m scripts.environment.java_exec backend/gradlew -p backend architectureTest --rerun-tasks --no-build-cache
```
