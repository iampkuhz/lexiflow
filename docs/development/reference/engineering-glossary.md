...
| Check | 一项具体检查能力；与命令 `delivery_gate check` 的条件核对场景区分 | [Verify](../change-delivery/verification.md) |
...
| Delivery Gate / Formal Gate | 独立身份下的正式送验、验证、审查与条件核对，不等同于本地命令成功 | [Delivery Gate](../change-delivery/delivery-gate.md) |
| validation | 执行冻结输入的必需检查；不要与只读 review 混用 | [独立验证](../change-delivery/delivery-gate.md#13-validate执行独立验证) |
| review | 复核 frozen diff 和 evidence，不重新运行交付检查 | [独立审查](../change-delivery/delivery-gate.md#14-review复核差异与证据) |
...
| producer / validator / reviewer | 分别产出实现、执行验证、独立审查；不同名称不等于不同真实身份 | [Delivery Gate](../change-delivery/delivery-gate.md) |
| callback / ack | callback 是终态信号投递，ack 是消费确认；两者均不等于验收 PASS | [Agent 交接](../agent-workflow.md) |
| submission | 绑定 Task、producer、报告和冻结输入的正式送验对象 | [submit](../change-delivery/delivery-gate.md#12-submit冻结送验输入) |
| report / record / receipt | report 描述检查事实；record 是持久化记录；receipt 在正式链中指被核对的证据记录，不随意互换 | [Delivery Gate](../change-delivery/delivery-gate.md) |
...
| fingerprint / hash DAG | 指纹绑定内容；hash DAG 绑定证据和依赖关系，不能靠文件名相同替代 | [条件核对](../change-delivery/delivery-gate.md#15-check核对条件而非重跑) |
| PASS / BLOCKED / FAIL | 唯一结果枚举，保持原词；具体判定由所属场景负责 | [排障](../troubleshooting.md) |
| fail closed | 缺少可信输入或证据时不放行，不意味着所有错误都归同一状态 | [Delivery Gate](../change-delivery/delivery-gate.md) |
