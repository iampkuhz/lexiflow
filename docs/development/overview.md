...
开发工作的主干是“实现 → Verify → 必要时正式 Acceptance”。环境准备是前置能力，Agent 协作是可选实现方式，格式与配置检查是可独立调用的叶子能力；它们不应被串成每次都必须走完的长链。
...
component "Acceptance CLI" as formal
}
package "场景与事实" #D1FAE5 {
component "Verification" as verification
component "Acceptance" as acceptance
component "Agents 运行事实" as agents
}
package "原子执行能力" #FEF3C7 {
component "模块自有检查" as modules
component "Environment" as environment
}
daily --> verification: S1 调用场景
formal --> acceptance: S2 调用场景
acceptance --> verification: S3 消费公共 API
acceptance --> agents: S4 读取真实事实
verification --> modules: S5 执行固定检查
verification --> environment: S6 读取环境能力
legend bottom
蓝：公开入口；绿：场景与事实；黄：原子执行能力
箭头表示调用或读取方向，S 编号是关系索引，不是阶段
endlegend
@enduml
```

上图是能力依赖，不是执行时序。Acceptance 消费 Verification 与 Agents 的公开能力；Verification 不读取 Task identity。Agents 产生真实执行事实，不签发正式验收。模块拥有自己的检查内容，全局执行器不应了解模块内部测试类。
