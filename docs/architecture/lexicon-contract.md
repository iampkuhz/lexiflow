...
**位置：** [架构总览](overview.md) → [观看流程](flows.md) → 共享词库合同。**上游：** 离线导入并发布版本；**下游：** Enrichment 查询候选。**失败：** 输入冲突拒绝发布、查询未命中返回无候选，不创建观看补全任务。

`Lexicon` 保存可跨内容复用的英文词汇事实；它不保存某次字幕采用哪个义项、提示是否展示、
用户行为或供应商载荷。`Enrichment` 只能通过公开查询合同读取已发布的版本。逻辑聚合身份和
持久化模型与初始化边界见[词库持久化模型](data-model.md)。

## 1.1. 可复用知识的最小单位：词条、lemma 与义项
...
导入格式、公式、来源审阅和操作命令见
[离线词库导入与预热](../development/operations/lexicon-import.md)。
