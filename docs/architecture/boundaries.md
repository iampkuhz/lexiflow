...
LexiFlow 保留两个业务领域和八个 Gradle 叶项目：共享词汇知识与通用字幕提示是业务边界；用例协调、技术适配、运行入口和验证各自承担非业务责任。产品不维护账号、个人档案、学习行为或跨设备同步。
...
| 语境释义 | 语义能力只服务提示，不拥有独立业务事实。 | Enrichment 声明任务合同和标准结果；最外层的 `platform:infrastructure` 调用模型或模拟器。结果绑定本次 `CaptionContext`。 |
...
这三类概念不组成 `content`、`semantic` 或 `foundation` Gradle 项目。只有出现内容库、独立语义产品或稳定的无领域共享合同，才按新增事实、消费者和依赖证据重新评估边界。
...
| 应用 | `:application:workflow` | 1 |
| 基础设施 | `:platform:infrastructure` | 1 |
...
package "最外层基础设施" #FEF3C7 {
  component "platform:infrastructure" as infrastructure
}
...
- 位于最外层的 `platform:infrastructure` 承接模型、数据库、缓存、观测和安全等技术实现，但不拥有领域事实或发起领域决策。
...
| `:application:workflow` | 将一次交互或后台任务按可靠顺序完成。 | 持久工作、尝试、取消、恢复、投递状态。 | 不判断词义或提示价值，不直接依赖 Postgres、Redis 或模型 SDK。 |
| `:platform:infrastructure` | 为上层提供具体技术集成。 | 模型/模拟器调用、Postgres、Redis、观测和安全接入。 | 不调用 Enrichment 决策，不改工作状态机、提示规则或重试预算，不因数据库访问而成为所有表的 owner。 |
...
| `workflow` / `platform:infrastructure` | Workflow 定义**需要什么保证**；基础设施用技术实现**怎样做到**。 | workflow 要求“只有持久交接成功才能返回 pending”；基础设施用事务或条件更新实现。 |
