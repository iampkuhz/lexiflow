...
class TestAuthorityFreshness(unittest.TestCase):
 ...
 def test_caller_fabricated_expiry_is_not_a_valid_authority_contract(self,m):
  import json
  p=self.f.root/"tmp/quality/delivery-gate/validations"/self.f.validation_id/"record.json";v=json.loads(p.read_text());v["authority"]["expires_at"]="2000-01-01T00:00:00Z";from scripts.delivery_gate.records import content_hash;v["content_hash"]=content_hash(v)
