...
from scripts.delivery_gate.records import RecordError,delivery_gate_locator,content_hash,list_layer,list_submissions,load_layer,load_submission,publish_json,read_json
from scripts.delivery_gate.producer import resolve_producer
from scripts.delivery_gate.validate import ValidationError, _verify_producer
from tests.delivery_gate.fixtures import DeliveryGateFixture,PRODUCER_SESSION,mock_authority,make_mock_runtime
class TestRecordSafety(unittest.TestCase):
 def test_concurrent_publication_has_one_winner(self):
  with tempfile.TemporaryDirectory() as tmp:
   root=Path(tmp);rid=str(uuid.uuid4());locator=delivery_gate_locator("submissions",rid)
   def write(i):
 ...
 def test_dangling_record_directories_are_not_treated_as_empty(self):
  with tempfile.TemporaryDirectory() as tmp:
   root=Path(tmp);base=root/"tmp/quality/delivery-gate";base.mkdir(parents=True)
 ...
 def test_publication_does_not_follow_existing_parent_symlink(self):
  with tempfile.TemporaryDirectory() as tmp,tempfile.TemporaryDirectory() as outside:
   ...
   with self.assertRaisesRegex(RecordError,"unsafe-path"):publish_json(root,delivery_gate_locator("submissions",rid),{"writer":1})
   self.assertEqual(list(Path(outside).iterdir()),[])
 def test_link_side_effect_error_rolls_back_owned_final(self):
  with tempfile.TemporaryDirectory() as tmp:
   root=Path(tmp);(root/"tmp/quality/delivery-gate/submissions").mkdir(parents=True);rid=str(uuid.uuid4());locator=delivery_gate_locator("submissions",rid);real_link=os.link
   def link_then_error(*args,**kwargs):real_link(*args,**kwargs);raise OSError("injected after link")
   with patch("scripts.delivery_gate.records.os.link",side_effect=link_then_error):
...
class TestDependencyDagAndApproval(unittest.TestCase):
 ...
 def test_multilevel_hash_update_and_explicit_approval(self):
  ...
  publish_json(self.f.root,"tmp/quality/delivery-gate/approvals/G1.json",approval)
  ...
  path=self.f.root/delivery_gate_locator("checks",ca["check_id"]);value=json.loads(path.read_text());value["reason"]="changed";value["content_hash"]=content_hash(value);path.chmod(0o600);path.write_text(json.dumps(value,separators=(",",":"),sort_keys=True));path.chmod(0o400)
