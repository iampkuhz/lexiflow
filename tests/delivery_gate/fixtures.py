...
class DeliveryGateFixture:
 ...
 def create_submission(self,**_):
  ...
  changed=report["scope_review"]["changed_files"];snaps={x:{"state":"present","sha256":sha256_bytes((self.root/x).read_bytes())} for x in changed};patch=publish_bytes(self.root,f"tmp/quality/delivery-gate/submissions/{sid}.diff.patch",b"fixture diff")
  runtime=make_mock_runtime(PRODUCER_SESSION);authority=mock_authority(PRODUCER_SESSION)
  record={"schema_version":"lexiflow.delivery-gate-submission.v4","submission_id":sid,"task_requirements":requirements,"change_report":desc,"scope_confirmation":self.report_id,"scope_base":"HEAD","diff":patch,"submitter_identity":runtime.context,"runtime_proof":runtime.proof,"authority":authority,"producer":{"kind":"current-codex-task","identity":runtime.context,"runtime_proof":runtime.proof,"authority":authority},"verification_freeze":freeze,"changed_file_snapshots":snaps,"created_at":"2026-09-21T00:00:00Z"};publish_json(self.root,delivery_gate_locator("submissions",sid),record);return record
