...
class TestCheck(unittest.TestCase):
 def setUp(self):self.f=DeliveryGateFixture();self.f.create_submission();self.f.create_validation();self.f.create_review()
 def tearDown(self):self.f.cleanup()
 def test_hash_tampering_is_blocked(self):
  p=self.f.root/"tmp/quality/delivery-gate/validations"/self.f.validation_id/"record.json";v=json.loads(p.read_text());v["result"]="FAIL";p.chmod(0o600);p.write_text(json.dumps(v));p.chmod(0o400)
 ...
 def test_missing_review_is_blocked(self):
  other=DeliveryGateFixture();other.create_submission();other.create_validation();r=check_conditions(other.root,submission_id=other.submission_id);self.assertEqual(r["reason"],"review-missing");other.cleanup()
 def test_missing_bound_validation_evidence_cannot_pass(self):
  (self.f.root/"tmp/quality/delivery-gate/validations"/self.f.validation_id/"report.json").unlink()
