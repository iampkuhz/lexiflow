...
def project(policy, rule):
    ...
    return {
        key: (
            ...
            else copy.deepcopy(lookup(policy, source))
        )
...
def replace_block(text, path, value):
    ...
    # collection 结束于下个同级 token 之前，不能吞掉同级字段所在行。
    end = node.end_mark.line + (
        1 if isinstance(node, yaml.ScalarNode) or node.flow_style else 0
    )
            ...
            json.dumps(
                {
                    ...
                    "drift": drift,
                    "updated": bool(args.write and drift),
                },
                ensure_ascii=False,
            )
        )
