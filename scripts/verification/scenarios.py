...
def freeze_inputs(
    root: str | Path = ".", *, required_check_ids: tuple[str, ...] | list[str] = ()
) -> dict[str, Any]:
    """返回基线与固定 ID 的完整确定性输入闭包。"""
    ...
    try:
        ...
        selected, _baseline, _partial = _select_repository_checks(
            declarations["checks"], None, tuple(required_check_ids)
        )
...
def _validate_frozen_inputs(
    ...
    if not isinstance(freeze, dict):
        return None
    if (
        freeze.get("schema_version") != "lexiflow.verification-freeze.v1"
