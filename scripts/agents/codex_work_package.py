"""Codex Work Package 的规范不可变发布器。先校验整个 caller contract 和宿主身份，再按 Task 发布 outcome、completion 与 signal；部分发布不得复用。"""
    ...
    for index, part in enumerate(parts):
        ...
        try:
            ...
            raise CodexWorkPackageError(
                "artifact-missing", f"missing path: {locator}"
            ) from None
        if stat.S_ISLNK(mode):
            raise CodexWorkPackageError(
                "unsafe-locator", f"symlink path component: {locator}"
            )
        if index != len(parts) - 1 and not stat.S_ISDIR(mode):
            raise CodexWorkPackageError(
                "unsafe-locator", f"non-directory ancestor: {locator}"
            )
    return current
...
def _mkdir(root: Path, locator: str) -> Path:
    ...
    for part in locator.split("/"):
        ...
        if stat.S_ISLNK(mode) or not stat.S_ISDIR(mode):
            raise CodexWorkPackageError(
                "unsafe-locator", f"unsafe directory: {locator}"
            )
    return current
...
def _write_exclusive(path: Path, content: bytes) -> None:
    ...
    try:
        ...
        raise CodexWorkPackageError(
            "run-collision", f"immutable path already exists: {path.name}"
        ) from None
    try:
    ...
    def visit(value: Any) -> None:
        if isinstance(value, dict):
            ...
            if (
                ...
                and task_id.startswith("LF-TSK-")
                and "task_version" in value
            ):
                if task_id in result:
                    raise CodexWorkPackageError(
                        "catalog-invalid", f"duplicate task: {task_id}"
                    )
...
def _identity(runtime: Mapping[str, Any]) -> dict[str, str]:
    if not isinstance(runtime, Mapping) or set(runtime) != set(_IDENTITY_FIELDS):
        raise CodexWorkPackageError(
            ...
            "trusted runtime context must contain only identity fields",
        )
    result: dict[str, str] = {}
    for field in _IDENTITY_FIELDS:
        ...
        if not isinstance(value, str) or not value:
            raise CodexWorkPackageError(
                "runtime-context-invalid", f"{field} is missing"
            )
        result[field] = value
    if (
        not _UUID.fullmatch(result["run_id"])
        or not _UUID.fullmatch(result["session_id"])
        or not _UUID.fullmatch(result["parent_session_id"])
    ):
        raise CodexWorkPackageError(
            "runtime-context-invalid", "runner session and run identities must be UUIDs"
        )
    if (
        not _AGENT_ID.fullmatch(result["agent_id"])
        or result["client"] != "codex"
        or result["parent_client"] != "codex"
    ):
        raise CodexWorkPackageError(
            "runtime-context-invalid", "runner identity is not a Codex identity"
        )
    return result
...
def _caller(caller: Mapping[str, Any]) -> dict[str, Any]:
    ...
    # parent_client 是稳定的 caller 路由约束；其余字段均是 runner 拥有的执行身份。
    forbidden = (set(_IDENTITY_FIELDS) - {"parent_client"}) & set(caller)
    if forbidden:
        raise CodexWorkPackageError(
            "caller-contract-invalid", "caller owns no runner identity"
        )
    # 现有 projection 校验器负责完整的 caller 结构检查。
    return dict(caller)


def _validate_current(caller: dict[str, Any], tasks: dict[str, dict[str, Any]]) -> None:
    """将 caller contract 和每个目标 Task 与当前 Catalog 对齐，包括版本、owner、范围和验收。任何单项漂移都会在发布前拒绝整个包。"""
    for task_id in caller.get("task_ids", []):
        ...
        checks = {
            ...
            "acceptance_criteria": caller["acceptance_by_task"]
            .get(task_id, {})
            .get("acceptance_criteria"),
            "acceptance_evidence": caller["acceptance_by_task"]
            .get(task_id, {})
            .get("acceptance_evidence"),
            "validation_command": caller["validation_commands"].get(task_id),
        }
        expected = {
            "task_version": current.get("task_version"),
            "change_version": current.get("change_version"),
            "owner": current.get("owner"),
            "deliverable": current.get("deliverable"),
            "acceptance_criteria": current.get("acceptance_criteria"),
            "acceptance_evidence": current.get("acceptance_evidence"),
            "validation_command": current.get("validation_command"),
        }
        if checks != expected:
            raise CodexWorkPackageError(
                "catalog-drift", f"current task contract drifted: {task_id}"
            )
        allowed = current.get("allowed_files")
        forbidden = current.get("forbidden_files")
        if not isinstance(allowed, list) or not isinstance(forbidden, list):
            raise CodexWorkPackageError(
                "catalog-invalid", f"scope unavailable: {task_id}"
            )
        # 只有每个 Task 声明的文件都包含在包范围内，包范围才允许更宽。
        package_allowed = normalize_scope_string(caller["allowed_files"])
        package_forbidden = normalize_scope_string(caller["forbidden_files"])
        if any(
            not any(
                dispatch_path_contains(container, item) for container in package_allowed
            )
            for item in allowed
        ) or any(
            not any(
                dispatch_path_contains(container, item)
                for container in package_forbidden
            )
...
def _projection(
    caller: dict[str, Any], identity: dict[str, str], task_id: str
) -> dict[str, Any]:
    ...
    projection = {
        ...
        "task_ids": list(caller["task_ids"]),
        "target_task_id": task_id,
        "task_id": task_id,
        "task_source": "planning/workstreams.yaml",
        "task_version": caller["task_versions"][task_id],
        "change_version": caller["change_versions"][task_id],
        "allowed_files": caller["allowed_files"],
        "forbidden_files": caller["forbidden_files"],
        "expected_output": caller["expected_outputs_by_task"][task_id],
        "acceptance_criteria": item["acceptance_criteria"],
        "acceptance_evidence": item["acceptance_evidence"],
        "validation_command": caller["validation_commands"][task_id],
...
def _outcome(
    task_id: str, supplied: Any, repo_root: Path, command: str
) -> tuple[dict[str, Any], list[dict[str, str]]]:
    """将单个 Task 的原生执行事实规整为发布 outcome；校验验证命令和证据定位符，不从日志或自报状态推断 PASS。"""
    if not isinstance(supplied, Mapping) or set(supplied) != {
        ...
        raise CodexWorkPackageError(
            "outcome-invalid",
            f"{task_id} must have result_fields, artifacts and validation_summary",
        )
    fields = supplied["result_fields"]
    if not isinstance(fields, Mapping) or set(fields) != set(REQUIRED_RESULT_FIELDS):
        raise CodexWorkPackageError(
            "outcome-invalid", f"{task_id} six result fields are incomplete"
        )
    ...
    for index, item in enumerate(artifacts):
        if not isinstance(item, Mapping) or set(item) != {"locator", "sha256"}:
            raise CodexWorkPackageError(
                "outcome-invalid", f"{task_id} artifact {index} is invalid"
            )
        ...
        if digest != actual:
            raise CodexWorkPackageError(
                "artifact-hash-mismatch", f"{task_id} artifact hash mismatch: {locator}"
            )
    ...
    if not isinstance(summary, Mapping) or summary.get("command") != command:
        raise CodexWorkPackageError(
            "outcome-invalid", f"{task_id} validation_summary is invalid"
        )
    ...
    if exit_code is not None and (
        ...
        raise CodexWorkPackageError(
            "outcome-invalid", f"{task_id} exit_code must be an integer or null"
        )
    if fields["status"] == "PASS" and (
        exit_code != 0
        or fields["validation"]["status"] != "PASS"
        or any(value != "PASS" for value in fields["effect_checks"].values())
    ):
        raise CodexWorkPackageError(
            "outcome-invalid", f"{task_id} PASS requires completed validation"
        )
    return {
        ...
        "validation_summary": dict(supplied["validation_summary"]),
    }, descriptors
...
def _read_descriptor(
    repo_root: Path, descriptor: Any, expected_locator: str
) -> dict[str, Any]:
    if (
        not isinstance(descriptor, dict)
        or set(descriptor) != {"locator", "sha256", "bytes"}
        or descriptor["locator"] != expected_locator
    ):
        raise CodexWorkPackageError(
            "completion-invalid", "artifact must use its fixed canonical locator"
        )
...
class CodexWorkPackagePublisher:
    ...
    def publish(
        ...
        if (
            ...
            raise CodexWorkPackageError(
                "outcome-invalid", "outcomes must exactly cover caller task_ids"
            )
        # 写入前一次性构建全部 projection，复用现有校验器核对完整合同。
        projections = {
            task_id: _projection(caller, identity, task_id) for task_id in task_ids
        }
        ...
        prepared_outcomes = {
            task_id: _outcome(
                ...
                outcomes[task_id],
                self.repo_root,
                caller["validation_commands"][task_id],
            )
        ...
        if run_root.exists() or run_root.is_symlink():
            # runner 拥有的命令捕获可先准备，但 publisher 绝不续写部分发布的 Task 布局。
        ...
        try:
            for task_id in task_ids:
                ...
                completion = {
                    ...
                    "artifacts": {
                        "task_projection": _descriptor(
                            projection_locator, projection_bytes
                        ),
                        "outcome": _descriptor(outcome_locator, outcome_bytes),
                        "external": external,
                    },
                ...
                signal = {
                    ...
                    "artifact_locators": {
                        "completion": _descriptor(completion_locator, completion_bytes),
                        "outcome": _descriptor(outcome_locator, outcome_bytes),
                    },
                    "validation_commands": [caller["validation_commands"][task_id]],
                    "blocking_findings": [],
                }
                ...
                package_tasks[task_id] = {
                    ...
                    "artifacts": {
                        ...
                        "outcome": _descriptor(outcome_locator, outcome_bytes),
                        "completion": _descriptor(completion_locator, completion_bytes),
                        "signal": _descriptor(signal_locator, signal_bytes),
                    },
                }
            ...
            completion = {
                ...
                "work_package_id": caller["work_package_id"],
                "task_ids": task_ids,
                "task_versions": caller["task_versions"],
                "change_versions": caller["change_versions"],
                "identity": identity,
                "status": _aggregate(statuses),
                "tasks": package_tasks,
                "validation_summary": {
                    task_id: outcomes[task_id]["validation_summary"]
                    for task_id in task_ids
                },
            }
            completion_bytes = canonical_json_bytes(completion)
    ...
    def verify(self, run_id: str) -> dict[str, Any]:
        """只读核验整包不可变发布物、身份、逐 Task 内容与哈希链。输入为本地 run 标识；返回核验结果，不补写缺失记录，也不重新运行验证命令。"""
        if not isinstance(run_id, str) or not _UUID.fullmatch(run_id):
            raise CodexWorkPackageError(
                "runtime-context-invalid", "run_id must be a UUID"
            )
        ...
        if (
            ...
            raise CodexWorkPackageError(
                "completion-invalid",
                "task map must cover the ordered unique Task list exactly",
            )
        ...
        for task_id in task_ids:
            ...
            if (
                ...
                or set(entry) != {"status", "artifacts"}
                or entry["status"] not in {"PASS", "BLOCKED", "FAIL"}
            ):
                raise CodexWorkPackageError(
                    "completion-invalid", f"invalid task status: {task_id}"
                )
            ...
            if not isinstance(artifacts, dict) or set(artifacts) != {
                ...
                raise CodexWorkPackageError(
                    "completion-invalid", f"missing canonical artifacts: {task_id}"
                )
            task_locator = f"tmp/quality/codex-work-packages/{run_id}/tasks/{task_id}"
            projection = _read_descriptor(
                self.repo_root,
                artifacts["task_projection"],
                f"{task_locator}/task-projection.json",
            )
            ...
            if projection != expected_projection or (
                caller is not None and caller != projected_caller
            ):
                raise CodexWorkPackageError(
                    "completion-invalid", "per-Task projection binding drifted"
                )
            caller = projected_caller
            _validate_current(caller, catalog)
            if any(
                completion[field] != caller[field]
                for field in (
                    "work_package_id",
                    "task_ids",
                    "task_versions",
                ...
                raise CodexWorkPackageError(
                    "completion-invalid",
                    "package contract differs from per-Task projection",
                )
            outcome = _read_descriptor(
                self.repo_root, artifacts["outcome"], f"{task_locator}/outcome.json"
            )
            supplied = {
                key: outcome.get(key)
                for key in ("result_fields", "artifacts", "validation_summary")
            }
            checked_outcome, external = _outcome(
                task_id,
                supplied,
                self.repo_root,
                caller["validation_commands"][task_id],
            ...
            if outcome != checked_outcome or entry["status"] != outcome["status"]:
                raise CodexWorkPackageError(
                    "completion-invalid", "outcome fields or status drifted"
                )
            task_completion = _read_descriptor(
                self.repo_root,
                artifacts["completion"],
                f"{task_locator}/completion.json",
            )
            expected_completion = {
                "schema_version": COMPLETION_SCHEMA_VERSION,
                "work_package_id": caller["work_package_id"],
                ...
                "validation_summary": outcome["validation_summary"],
                "artifacts": {
                    "task_projection": artifacts["task_projection"],
            ...
            if task_completion != expected_completion:
                raise CodexWorkPackageError(
                    "completion-invalid", "task completion binding drifted"
                )
            signal = _read_descriptor(
                self.repo_root, artifacts["signal"], f"{task_locator}/signal.json"
            )
            expected_signal = {
                "schema_version": SIGNAL_SCHEMA_VERSION,
                "status": outcome["status"],
            ...
            if signal != expected_signal:
                raise CodexWorkPackageError(
                    "completion-invalid", "task signal binding drifted"
                )
            summaries[task_id] = outcome["validation_summary"]
            statuses.append(entry["status"])
        if completion["validation_summary"] != summaries or completion[
            "status"
        ] != _aggregate(statuses):
            raise CodexWorkPackageError(
                "completion-invalid", "aggregate status is invalid"
            )
        return {
            "status": "PASS",
            "work_package_id": completion["work_package_id"],
            "task_ids": task_ids,
            "run_id": run_id,
            "artifact_locators": {
                "package_completion": {
                    "locator": locator,
                    "sha256": sha256_file_strict(self.repo_root, locator),
                }
            },
            "validation_commands": [],
            "blocking_findings": [],
...
def build_codex_main_task_projection(
    ...
    *,
    ...
    """从当前 Catalog 和宿主绑定构造单 Task Main projection。只使用 runner 身份与当前任务合同，调用者声明不能取得执行权。"""
    ...
    if any(field not in record for field in required):
        raise CodexWorkPackageError(
            "catalog-invalid", f"Main task contract is incomplete: {task_id}"
        )
    projection = {
        ...
        "allowed_files": ", ".join(record["allowed_files"]),
        "forbidden_files": ", ".join(record["forbidden_files"]),
        "expected_output": record["deliverable"],
        "acceptance_criteria": record["acceptance_criteria"],
        "acceptance_evidence": record["acceptance_evidence"],
