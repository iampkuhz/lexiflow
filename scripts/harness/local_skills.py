...
def inspect(root: Path, source: Path):
    ...
    for item in policy['skills']:
        ...
        if link.is_symlink():
            ...
            status = 'target-conflict'
        results.append({'name': name, 'status': status, 'entry': str(entry.resolve()),
    ...
    if mode == 'link' and not bad:
        for item in results:
            if item['status'] == 'ready':
                ...
                link.symlink_to(Path(item['source_directory']), target_is_directory=True)
                item['status'] = 'linked'
    complete = all(x['status'] == 'linked' for x in results)
    return {'status': 'FAIL' if any(x['status'] == 'target-conflict' for x in results)
            else 'PASS' if complete else 'BLOCKED', 'skills': results}
