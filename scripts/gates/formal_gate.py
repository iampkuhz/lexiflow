"""Formal Gate entrypoint for planning, validation receipt publication, and status."""
...
def _task_validation_result(plan: dict[str, Any], execution: dict[str, Any],
                            repository_verification: dict[str, Any]) -> tuple[str, list[str]]:
    attested = plan["subject"]["main_agent_attestation"]["result_fields"]
    statuses = [execution.get("run_status"), repository_verification.get("result"), attested.get("status"), attested.get("validation", {}).get("status")]
...
def _build_task_validation_receipt(
    *, plan: dict[str, Any], run_id: str, plan_artifact: dict[str, str],
    ...
    result, reasons = _task_validation_result(plan, execution, repository_verification)
    checks = []
    ...
    receipt = {
        ...
        "validation": {
            "repository_verification": copy.deepcopy(repository_verification),
...
def _publish_task_validation(
    *, store: ImmutableReceiptStore, plan: dict[str, Any], run_id: str,
    ...
    execution: dict[str, Any], now: Callable[[], str], started_at: str | None = None,
    repository_verification: dict[str, Any] | None = None, **_: Any,
) -> dict[str, Any]:
    if not isinstance(repository_verification, dict) or repository_verification.get("result") != "PASS":
        raise GateCliError("repository-baseline-required", "TASK_VALIDATION requires an independent PASS repository verification")
...
def _publish_failed_receipt(
    *, store: ImmutableReceiptStore, plan: dict[str, Any], run_id: str,
    plan_artifact: dict[str, str], process_identity: dict[str, Any],
    started_at: str, now: Callable[[], str], code: str, result: str = "FAIL",
    ...
    receipt = {
        ...
        "canonical_rerun": _canonical_rerun(plan),
        "result": result if result in ("BLOCKED", "FAIL") else "FAIL",
        "reasons": [code],
    }
    if repository_verification is not None:
        receipt["repository_verification"] = copy.deepcopy(repository_verification)
    store.publish_receipt(receipt)
    return receipt
...
def run_gate(
    ...
    repository_verifier: Callable[[str | os.PathLike[str]], dict[str, Any]] | None = None,
) -> dict[str, Any]:
    ...
    try:
        ...
        if receipt_kind == "TASK_VALIDATION":
            if repository_verifier is None:
                # Test compilers are a zero-I/O seam. Production always uses
                # the canonical compiler and therefore the real verifier.
                if compiler is compile_plan:
                    from scripts.gates.repository_verify import verify as verify_repository
                    repository_verifier = verify_repository
                else:
                    repository_verifier = lambda _root: {"result": "PASS", "scope": "test-compiler-seam"}
            repository_verification = repository_verifier(repo_root)
            baseline_result = repository_verification.get("result") if isinstance(repository_verification, dict) else None
            if baseline_result != "PASS":
                return _publish_failed_receipt(
                    store=store, plan=plan, run_id=run_id,
                    plan_artifact={"locator": plan_artifact.locator, "sha256": plan_artifact.sha256},
                    process_identity=process_identity, started_at=started_at, now=now,
                    code="repository-baseline-not-pass", result=baseline_result or "FAIL",
                    repository_verification=repository_verification if isinstance(repository_verification, dict) else None,
                )
        execution = (
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="scripts/gates/formal_gate.py")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = getattr(exc, "result", getattr(exc, "status", "FAIL"))
