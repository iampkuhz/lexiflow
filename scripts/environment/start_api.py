"""Interactive local API launcher; never terminate listeners without confirmation."""
...
class StartupBlocked(RuntimeError):
    """A port cannot safely be reused by this invocation."""


def listener_pids(port: int) -> set[int]:
    """Inspect TCP listeners only, not outbound connections or full command lines."""
...
def process_identity(pid: int) -> str:
    """Bind consent to PID, start time, owner and executable, without reading argv."""
    result = subprocess.run(
...
def assert_bindable(port: int) -> None:
    """Detect listeners invisible to lsof as well; unknown ownership never authorizes kill."""
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as probe:
        probe.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        try:
            probe.bind(("127.0.0.1", port))
        except OSError as exc:
            raise StartupBlocked(f"127.0.0.1:{port} 仍不可绑定；未启动，请手动检查占用或权限。") from exc


def prepare_port(port: int) -> None:
    """Prompt once, revalidate identities, signal only approved PIDs and await release."""
    ...
    while remaining := listener_pids(port):
        ...
        if time.monotonic() >= deadline:
            raise StartupBlocked("10 秒内端口未释放；不会强制 kill -9，请手动停止后重试。")
        time.sleep(0.1)
    assert_bindable(port)


def main(arguments: Sequence[str] | None = None) -> int:
    """Start the fixed local API command only after a successful port preflight."""
    parser = argparse.ArgumentParser(description="本机 API 启动；端口冲突时交互确认是否终止旧进程。")
    parser.add_argument("--port", type=int, default=18080)
    parser.add_argument("--database-url", help="可选 JDBC 地址；也可使用 SPRING_DATASOURCE_URL 环境变量。")
    ...
    try:
        ...
        command = [str(root / "backend/gradlew"), "-p", str(root / "backend"), ":apps:api:bootRun",
                   f"--args=--server.address=127.0.0.1 --server.port={args.port}"]
        os.execvpe(command[0], command, environment)
    except (StartupBlocked, JavaRuntimeError, OSError, subprocess.SubprocessError) as exc:
        # Never print arbitrary subprocess output, environment values or command argv.
        message = str(exc) if isinstance(exc, (StartupBlocked, JavaRuntimeError)) else "本机工具执行失败，请检查 lsof、ps、Gradle 与运行权限。"
