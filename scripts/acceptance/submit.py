    ...
    *,
    ...
    try:
        patch_descriptor = publish_bytes(
            repo,
            f"tmp/quality/acceptance/submissions/{submission_id}.diff.patch",
            patch,
        )
        created = datetime.now(UTC).isoformat(timespec="seconds").replace("+00:00", "Z")
        record = {
            "schema_version": "lexiflow.acceptance-submission.v4",
        ...
        published = publish_json(
            repo, acceptance_locator("submissions", submission_id), record
        )
    except RecordError as exc:
        raise _err(exc) from None
    return {
