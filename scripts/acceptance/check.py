"""只读评估正式 Acceptance 条件；只核对 receipt、依赖和证据，不运行交付命令。"""
...
def _records(
    repo: Path, submission: dict[str, Any]
) -> tuple[dict[str, Any] | None, dict[str, Any] | None, dict[str, Any] | None]:
    sid = submission["submission_id"]
    validation = _one(
        list_layer(repo, "validations", "submission_id", sid), "validation", sid
    )
...
def _dependency_status(
    repo: Path, submission: dict[str, Any], visiting: set[str] | None = None
...
def check_conditions(root: str | Path, *, submission_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        result = (
            ...
            else "FAIL" if chain_errors else "BLOCKED"
        )
        reason = (
            ""
            if result == "PASS"
    ...
    record = {
        "schema_version": "lexiflow.acceptance-check.v3",
        "check_id": check_id,
        "submission_id": submission_id,
        "validation_id": validation["validation_id"],
        "review_id": review["review_id"],
    ...
    try:
        published = publish_json(repo, acceptance_locator("checks", check_id), record)
    except RecordError as exc:
