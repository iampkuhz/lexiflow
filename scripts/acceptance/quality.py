"""Run the complete acceptance-module regression suite and emit one JSON result."""
...
def evaluate_suite(suite: unittest.TestSuite) -> dict[str, object]:
    ...
    return {
        ...
        "reason": "" if passed else "acceptance-module-tests-incomplete",
...
def run(root: Path) -> dict[str, object]:
    suite = unittest.defaultTestLoader.discover(
        str(root / "tests" / "acceptance"), pattern="test_*.py", top_level_dir=str(root)
    )
