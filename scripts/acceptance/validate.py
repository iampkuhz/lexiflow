...
def validate(root: str | Path, *, submission_id: str) -> dict[str, Any]:
    ...
    try:
        report_descriptor = publish_bytes(
            repo,
            f"tmp/quality/acceptance/validations/{validation_id}/report.json",
            canonical_bytes(report),
        )
        record = {
            "schema_version": "lexiflow.acceptance-validation.v3",
        ...
        published = publish_json(
            repo, acceptance_locator("validations", validation_id), record
        )
    except RecordError as exc:
        raise ValidationError(exc.code, exc.detail) from None
    return {
