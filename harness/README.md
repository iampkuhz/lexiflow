...
全仓只维护最新逻辑与架构；废弃接口、配置、数据结构及消费者同步替换，不保留历史兼容分支。数据库只维护唯一最新 SQL，结构变化显式重建本项目开发库并完整重新导入，不维护递增迁移链。API 启动不得隐式清库；不得触及其他项目数据。来源绑定、资料发布身份、事务一致性、隐私和验证护栏仍有效，不把它们当作兼容负担移除。资料重建后不得与缓存或本机偏好的旧身份混淆。
...
的静态一致性。`program.catalog_mode: planning-only` 允许尚未分解且无任务、阶段 Gate 或自动推进字段的目录通过静态检查；这不构成产品派发或正式验收资格。任务必须先按已确认阶段边界明确分解，旧任务 ID 和旧收据不能代表新阶段。
