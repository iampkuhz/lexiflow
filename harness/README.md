...
`--write` 只更新声明的投影字段，保留其他手写内容；`--check` 不修改文件。文档检查验证链接、锚点、图源围栏和入口大小，不替代中文语义、图文一致性或视觉审查。
...
修改图先读 PlantUML skill；仅维护技能本身才读治理 skill。图源始终在 Markdown 内，生成图包在 `tmp/`。IDEA 设置中启用 Markdown 的 PlantUML 扩展，在正文预览核对图像；独立 PUML 预览不构成正文验收。
